...
Check for an unlayered `summary { display: ... }` css rule elsewhere on the page
...
The most common source of this behavior is a `position` with a `z-index`, but there are plenty of other, subtler ways to create new stacking
